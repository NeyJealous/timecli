...
if [[ -n "$ORIGINAL_DATA_SOURCE" ]]; then
  ...
  if python3 -c "import UnityPy" >/dev/null 2>&1; then
    ...
    echo "3/5 Extracting private Flak/Rocket presentation..."
    ...
    echo "3/5 Extracting private click-weapon firing audio..."
    python3 "$SCRIPT_DIR/extract-private-click-weapon-audio.py" \
      "$ORIGINAL_DATA_SOURCE" \
      "$PRIVATE_RESOURCES"
  else
